...
#line 2 "lexer1.l"
	...
	void insert_SymbolTable_nest(char *s, int nest)
	{
		...
		if(search_SymbolTable(s) && ST[search_SymbolTable(s)].nested_val == 9999)
		{
			//printf("mlkjhad %d\n", nest);
             int pos = 0;
             int value = hash(s);
			 value = value%1000;
			for (int i = value + 1 ; i!=value ; i = (i+1)%1000)
			{
				if(ST[i].exist == 0)
				{
					pos = i;
